...
class MatMulGenerator(BaseGenerator):
    ...
    def generate(self):
        ...
        self._generate_vec_vec_sub(self.fixtures_path / "vec_vec_sub")
        self._generate_vec_vec_mul(self.fixtures_path / "vec_vec_mul")
    ...
    def _generate_vec_vec_sub(self, save_path):
        os.makedirs(save_path, exist_ok=True)

        i = 0
        metadata = []
        for size in [5, 512, 1024]:
            for dtype in ['float32']:
                ...
                expected = torch.sub(vec_a, vec_b)
                expected_save_path = save_path / f"{i}_expected.bin"
                self.save_tensor(expected, expected_save_path)
    ...
    def _generate_vec_vec_mul(self, save_path):
        os.makedirs(save_path, exist_ok=True)
        ...
        for size in [5, 512, 1024]:
            for dtype in ['float32']:
                ...
                expected = torch.mul(vec_a, vec_b)
                expected_save_path = save_path / f"{i}_expected.bin"
                self.save_tensor(expected, expected_save_path)
